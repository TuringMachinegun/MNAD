...
class Memory(nn.Module):
    ...
    def forward(self, query, keys, train=True):
        ...
        if train:
            ...
            # read
            updated_query, softmax_score_query, softmax_score_memory = self.read(
                query, keys
            )
            # update
        ...
        # test
            ...
            # loss
            compactness_loss, query_re, top1_keys, keys_ind = self.gather_loss(
                query, keys, train
            )
    ...
    def pointwise_gather_loss(self, query_reshape, keys, gathering_indices, train):
        ...
        pointwise_loss = loss_mse(
            query_reshape, keys[gathering_indices].squeeze(1).detach()
        )
    ...
    def read(self, query, updated_memory):
        ...
        softmax_score_query, softmax_score_memory = self.get_score(
            updated_memory, query
        )

        query_reshape = query.contiguous().view(batch_size * h * w, dims)

        concat_memory = torch.matmul(
            softmax_score_memory.detach(), updated_memory
        )  # (b X h X w) X d
        updated_query = torch.cat(
            (query_reshape, concat_memory), dim=1
        )  # (b X h X w) X 2d
