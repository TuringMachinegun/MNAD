...
class Memory(nn.Module):
    def __init__(self, memory_size, feature_dim, key_dim, temp_update, temp_gather, task="prediction"):
        super().__init__()
        self.task = task
        # Constants
    ...
    def forward(self, query, keys, train=True):
        ...
        if train:
            if self.task == "prediction":
                ...
                losses = separateness_loss, compactness_loss
    ...
    def gather_loss(self, query, keys, train):
        ...
        # when training in a prediction task, the loss should be TODO
        if self.task == "prediction" and train:
            loss = torch.nn.TripletMarginLoss(margin=1.0)
            _, gathering_indices = torch.topk(softmax_score_memory, 2, dim=1)
            # 1st, 2nd closest memories
            pos = keys[gathering_indices[:, 0]]
        ...
        # in all other cases, loss is simply TODO (but we report more stuff if in prediction)
        _, gathering_indices = torch.topk(softmax_score_memory, 1, dim=1)
        ...
        if self.task == "prediction":
            return (
                gathering_loss,
                query_reshape,
                keys[gathering_indices].squeeze(1).detach(),
                gathering_indices[:, 0],
            )
